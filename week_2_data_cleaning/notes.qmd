...
1. avocado_clean.info()  #see column types, null, column names

2. avocado_clean.head()  #first few values

3. avocado_clean_city['location'].unique()    #unique values in a ::: {.columns}

4. print(filtered_city_df.isnull().sum())  # Confirm there are no NA rows

5. dictionaety: geo_type_map

new column: 'geo_type'

old dataframe: avocado_clean

reference column: 'location'

.map : uses dictionary to map new column

avocado_clean['geo_type'] = avocado_clean['location'].map(geo_type_map)

### filtered the dataframe to find unmapped geographies
#unmapped_rows = avocado_clean[avocado_clean['geo_type'].isnull()]
#unmapped_geographies = unmapped_rows['geography'].unique()
#print(unmapped_geographies)
```
